...
class Bagel:
    ...
    def _run_table(self, table_name, elt_type):
        ...
        log_file_name = os.path.join(
            "logs", f"{self.integration.name}_{table_name}_{self.get_full_date()}.log"
        )
        ...
        data = self.integration.get_data(
            table_name,
        ...
        for d in data:
            if not isinstance(d, list):
                ...
                # generate file_name
            file_name = self.format_json_blob_name(self.integration.name, table_name)

            self.write_json_to_blob(file_name, bytes(str(d), "utf-8"))
        ...
        with open(log_file_name, "rb") as logfile:
            self.write_json_to_blob(
                self.format_json_blob_name(self.integration.name, table_name, True),
                logfile,
            )
    ...
    def format_json_blob_name(self, system, table, log=False):
        ...
        if log:
            ...
            file_type = "data"
        file_name = f"{system}/{file_type}/{table}/{year}/{month}/{day}/{table}_{full_date}.json"
        return file_name

    def get_full_date(self):
        full_date = datetime.utcnow().strftime("%Y_%m_%dT%H_%M_%S_%fZ")
        return full_date
    ...
    def get_tables(yaml_file):
        with open(yaml_file, "r") as file:
            data = yaml.safe_load(file)
            tables = dict(data.items())["tables"]
        return tables
